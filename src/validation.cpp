// Copyright (c) 2009-2010 Sa_toshi Nakamoto
// Copyright (c) 2009-2016 The Bitcoin Core developers
// Copyright (c) 2022-2023 The Dogecoin Core developers
// Copyright (c) 2024-2025 The Dedcoin Core developers
...
#if defined(NDEBUG)
# error "Dedcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Dedcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetDedcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Dedcoin: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Dedcoin: BIP30 has been active since inception
    ...
    // Dedcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetDedcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Dedcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Dedcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Dedcoin: We probably want to disable this
    int nLockTimeFlags = 0;
