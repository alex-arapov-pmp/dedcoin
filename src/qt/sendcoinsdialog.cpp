...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    // Dedcoin: We don't use smart fees in the UI, so don't need to warn they're not available
...
void SendCoinsDialog::updateGlobalFeeVariables()
{
    if (ui->radioSmartFee->isChecked())
    {
        ...
        // Dedcoin: We manually set height well past the last hard fork here
        ui->confirmationTargetLabel->setText(GetDedcoinPriorityLabel(nPriority).c_str());
    }
    else
    {
...
void SendCoinsDialog::updateFeeLabel()
{
    ...
    int nPriority = ui->sliderSmartFee->value();
    CFeeRate feeRate = GetDedcoinFeeRate(nPriority);
    if (feeRate <= CFeeRate(0)) // not enough data => minfee
    {
        ...
        ui->fallbackFeeWarningLabel->setIndent(QFontMetricsF(ui->fallbackFeeWarningLabel->font()).horizontalAdvance("x"));
    }
    else
    {
        ui->labelPriority->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
        ...
        // Dedcoin: We don't use smart fees, so we don't have the data to estimate when it will get in
        ui->labelFeeEstimation->setText("");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Dedcoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
