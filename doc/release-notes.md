Dedcoin Core version 1.14.7 is now available from:

  <https://github.com/dedcoin/dedcoin/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for Dedcoin-Qt. Dedcoin-Qt users on
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/dedcoin/lists/dedcoin-releases>


Compatibility
==============

Dedcoin Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for Dedcoin Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside Dedcoin Qt, support for BIP-70 payment
...
inside Dedcoin Qt. By default, incoming BIP-70 payment requests and files or
...
dedcoin.conf or as an argument to Dedcoin-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside Dedcoin
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `dedcoin.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on Dedcoin Core since
...
to support Dedcoin parametrization.
...
```
dedcoin-cli help getblockstats
```
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/dedcoin/ltc-scrypt
