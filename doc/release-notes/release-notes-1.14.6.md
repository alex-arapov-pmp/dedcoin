Dedcoin Core version 1.14.6 is now available from:

  <https://github.com/dedcoin/dedcoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Dedcoin Core users - miners, services,
...
Dedcoin network from 1 DEDC to 0.01 DEDC. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 DEDC as
   change if possible.
2. If after signing there is more than 0.01 DEDC left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 DEDC, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 DEDC will not be accepted as valid by default.
...
```
dedcoin-cli addnode <address> remove
```
...
```
dedcoin-cli liststucktransactions
```
...
```
dedcoin-cli setmaxconnections <desired_amount>
```
...
```
dedcoin-cli rescan <start_block>
```
...
efforts to make Dedcoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Dedcoin Core.
