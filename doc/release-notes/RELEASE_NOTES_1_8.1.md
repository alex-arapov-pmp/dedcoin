...
Dedcoin Core 1.8.1 is primarily a bugfix release, bringing Dedcoin Core in
line with Bitcoin 0.9.3. Dedcoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Dedcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Dedcoin Core clients
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/dedcoin/dedcoin/issues/134).
...
This only affects values significantly below 1 Ded, however merchants may wish to round to
the nearest Ded on all transactions for now.
