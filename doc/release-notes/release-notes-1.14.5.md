Dedcoin Core version 1.14.5 is now available from:

  <https://github.com/dedcoin/dedcoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Dedcoin Core users, miners, services, relay
operators and wallet users are strongly recommended to upgrade.

Please report bugs using the issue tracker at github:

  <https://github.com/dedcoin/dedcoin/issues>

To receive security and update notifications, please watch reddit or Twitter:

  * https://www.reddit.com/r/dedcoindev/
  * @Dedcoin on Twitter for high priority announcements
  * @dedcoin\_devs on Twitter for updates on development work

Compatibility
==============

Dedcoin Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.

Dedcoin Core should also work on most other Unix-like systems but is not
...
most Dedcoin Core users.

### Remote Code Execution in Dedcoin QT (CVE-2021-3401)
...
**Dedcoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Dedcoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Dedcoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Dedcoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 DEDC/kb, and
* the recommended dust limit is 1 DEDC, and
* the recommended RBF increment is 0.001 DEDC.

### Wallet/UI Changes

* The user interface for selecting fees when transacting DEDC has been updated
  to give an idea of how much is being spent, rather than a block target. As
  Dedcoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Dedcoin.
...
* The default minimum transaction fee is now 0.01 DEDC per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 DEDC, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 DEDC.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 DEDC and sets the value under
...
The Berkley DB version used by Dedcoin Core has been updated to 5.3 (from 5.1)
...
per [SLIP44](https://github.com/farthinglabs/slips/blob/master/slip-0044.md).
...
performance of cryptographic routines within Dedcoin Core.
