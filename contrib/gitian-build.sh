...
# Dependencies
ossPatchUrl="https://depends.dedcoincore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.dedcoincore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.dedcoincore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.dedcoincore.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/dedcoin/dedcoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of dedcoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/dedcoin/dedcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/dedcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/dedcoin/gitian.sigs.git
    git clone https://github.com/dedcoin/dedcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/dedcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean dedcoin git directory because of old caching
    if [ -d inputs/dedcoin/ ]; then
        echo "Cleaning dedcoin directory..."
        rm -rf inputs/dedcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit dedcoin="$COMMIT" --url dedcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
