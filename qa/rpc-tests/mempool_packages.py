...
class MempoolPackagesTest(BitcoinTestFramework):
    ...
    def chain_transaction(self, node, parent_txid, vout, value, fee, num_outputs):
        send_value = farthing_round((value - fee)/num_outputs)
    ...
    def run_test(self):
        ...
        for x in reversed(chain):
            descendant_fees += mempool[x]['fee']
            if (x == chain[-1]):
                assert_equal(mempool[x]['modifiedfee'], mempool[x]['fee']+farthing_round(0.00002))
        ...
        send_value = farthing_round((value - fee)/2)
